...
# parser.add_argument('--get-B', nargs='+', type=str, default='0,15,20,30,45,60,75', help='path element - global curvature - Comma seperated list')
parser.add_argument('--get-B', type=str, default='0,15,20,30,45,60,75', help='path element - global curvature - Comma seperated list')
...
print(args.get_B)
narrow_B=[int(item) for item in args.get_B.split(',')]


################################################################################################################################################################
visual_diet_config={'root_directory':os.path.expanduser('/home/jovyan/work/Datasets/contour_integration/model-training/config_0/'),'get_B':narrow_B,'get_D':args.get_D,'get_A':args.get_A,'get_numElements':[12]}
...
def train_model(model, dataloaders, criterion, optimizer, scheduler=None, num_epochs=25,save_model_weights=False):
    ...
    if(save_model_weights):
        torch.save({
                ...
                }, './saved_model/model_' + training_config['base_model_name'].replace('_','-') + '_layer_' + training_config['layer_name'].replace('.','-')+'_mode_'+mode + '_beta_' + '-'.join([str(b) for b in get_B]).zfill(3) + '.pt')
